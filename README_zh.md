...
- **簡易 AED 模式** — 心律面板可在「進階 ACLS」（五心律選擇器）與「簡易 AED」之
  間切換；簡易 AED 只記錄「可電擊／不可電擊」的粗判結果，並在「建議電擊」後提供
  一鍵「已電擊」捷徑，適合無法判讀特定心律的施救者；所選模式會被記住並於重新載
  入後還原。
- **即時治療時間軸** — 反向時序的紀錄，每筆顯示時刻與經過時間；**滑動項目即可顯
  示刪除按鈕**以修正誤觸，所有摘要計數器皆保持一致（單一資料來源）。
- **適合手套與橫向操作** — 每個控制項皆符合 ≥56 px 的觸控下限並具防彈跳機制；版
  面會從手機直向（單欄）自動調整為平板／橫向的多區版面，時間軸常駐顯示。
...
src/
  ...
  hooks/               # useViewport（自適應版面）、useRhythmMode、useTapGuard
  persistence/         # localStorage 案件儲存與 UI 偏好儲存
  theme/               # 設計 token、日／夜主題、觸控下限常數
  styles/              # 全域樣式
